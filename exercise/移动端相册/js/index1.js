...
//列表状态样式
function init() {
    $('.list').css({width: '100%', left: 0});
    $('li').css({width: squareWidth, height: squareWidth});
    $('img').css({width: '100%', height: 'auto'});
}
init();
// 图片展开样式
function show() {
    $('.list').css({width: total * viewWidth, left: 0});
    $('li').css({width: viewWidth, height: viewHeight});
    radio();
}
// 
$('.list').on('tap', 'li', function () {
    ...
    $('.list').css({left: -index*viewWidth + 'px'});
...
function slider(num) {
    ...
    index += num;
    $('.list').animate({left: -index*viewWidth + 'px'}, 200);
}
// 判断图片宽高比是否超过屏幕宽高比
function radio() {
